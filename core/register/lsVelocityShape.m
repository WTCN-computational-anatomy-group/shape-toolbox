function result = lsVelocityShape(model, dv, r0, match0, mu, f, varargin)
...
%--------------------------------------------------------------------------
%
% FORMAT [ok, ...] = lsVelocityShape(model, dv, r0, match0, mu, f, ...)
%
% REQUIRED
% --------
...
% w    - weight of the shape-agnostic prior [0]
% > Shooting
...
% tplbnd - Boundary conditions (0=circulant)/[1=neumann]
% > Processing
...
%__________________________________________________________________________
    ...
    if isnan(reg0)
        if w ~= 1
            reg0 = (1-w) * lam * llPriorVelocity(r0, 'fast', 'vs', vs, 'prm', prm, 'bnd', bnd, 'debug', debug);
        else
            reg0 = 0;
        end
    end 
    if isnan(vel0)
        if w
            vel0 = w * llPriorVelocity(v0, 'fast', 'vs', vs, 'prm', prm, 'bnd', bnd, 'debug', debug);
        else
            vel0 = 0;
        end
    end
    ...
    for i=1:nit
        ...
        if strcmpi(matchmode, 'push')
            [pf, c, bb] = pushImage(ipsi, f, latmu, 'circ', ~tplbnd, 'par', par, 'loop', loop, 'debug', debug, 'output', {pf, c});
            match = llMatching(model, mu, pf, c, 'bb', bb, 'par', par, 'loop', loop, 'debug', debug);
        elseif strcmpi(matchmode, 'pull')
            if cat
                wa = pullTemplate(ipsi, mu, 'order', itrp, 'bnd', ~tplbnd, 'par', par, 'output', wa, 'debug', debug);
                wmu = reconstructProbaTemplate(wa, 'output', wmu, 'loop', loop, 'par', par, 'debug', debug);
            else
                wmu = pullTemplate(ipsi, mu, 'order', itrp, 'bnd', ~tplbnd, 'par', par, 'output', wmu, 'debug', debug);
            end
            match = llMatching(model, wmu, f, 'par', par, 'loop', loop, 'debug', debug);
        end
        r = r0 + dv / armijo;
        if w ~= 1
            reg = (1-w) * lam * llPriorVelocity(r, 'fast', 'vs', vs, 'prm', prm, 'bnd', bnd, 'debug', debug);
        else
            reg = 0;
        end
        if w
            vel = w * llPriorVelocity(v, 'fast', 'vs', vs, 'prm', prm, 'bnd', bnd, 'debug', debug);
        else
            vel = 0;
        end
        ll  = match + reg + vel;
