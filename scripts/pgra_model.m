...
% sigma0       - Initial residual variance [sqrt(1/(N-2))]
...
% _________________________________________________________________________
    ...
    opt.sigmadf = opt.N;
    if ~isfield(opt, 'sigma0')
        opt.sigma0 = sqrt(1/(opt.sigmadf - 2));
%         opt.sigma0 = 1;
    end
    ...
    switch lower(opt.model.name)
        case {'normal', 'gaussian', 'l2'}
            opt.model.sigma2 = zeros(1, size(model.mu, 4));
            fprintf('%10s | %10s | [ ', 'Noise', 'Normal');
            for k=1:size(model.mu, 4)
                sig = 0;
                c   = 0;
                for n=1:opt.N
                    f   = single(dat(n).f(:,:,:,k));
    ...
    % -----------
    % Lower bound
    model.lbaq = lbPrecisionZ(model.Aq, opt.N, opt.nq0);
    model.lbq  = lbAffine(dat, model, opt);
    model.llw  = llPriorSubspace(model.w, model.ww, opt.vs, opt.prm);
    model.lbaz = lbPrecisionZ(model.Az, opt.N, opt.nz0);
    ...
    for emit = 1:opt.emit
        ...
        if activated.affine
            ...
            % Update parameters
            % -----------------
            [dat, model] = pgraBatchProcess('FitAffine', dat, model, opt);

            % Update prior
            % ------------
            model.Aq = precisionZWishart(opt.nq0, model.qq + model.Sq, opt.N);
            model.regq = model.Aq;
        ...
        if activated.pg
            ...
            [dat, model] = pgraBatchProcess('GradHessSubspace', dat, model, opt);
            ...
            [~, model, dat] = lsSubspace(model.dw, model, dat, opt, 'pgra');
        ...
        if activated.pg
            ...
            % Update q(z)
            % -----------
            [dat, model] = pgraBatchProcess('FitLatent', dat, model, opt);
            ...
            Q = Q*U;
            iQ = iU*iQ;
            [model, dat] = pgraRotateAll(model, dat, opt, Q, iQ);
            model.Az = precisionZWishart(opt.nz0, model.zz + model.Sz, opt.N);
...
%             % Subject grad/hess
%             % -----------------
%             [dat, model] = pgraBatchProcess('GradHessSigma', dat, model, opt);
%             s = log(model.sigma);
%             model.gs = exp(s)   * model.gs;
%             model.hs = exp(2*s) * model.hs;
% 
%             % Prior grad/hess
%             % ---------------
%             model.gs = model.gs - exp(-2*s) + opt.sigmadf + 2;
%             model.hs = model.hs + 2*exp(-2*s);
%     %         model.gs = model.gs + (opt.sigmadf + 2)*model.sigma^(-1) ...
%     %                             - model.sigma^(-3);
%     %         model.hs = model.hs - (opt.sigmadf + 2)*model.sigma^(-2) ...
%     %                             + 3 * model.sigma^(-4);
% 
%             % Search direction
%             % ----------------
%             model.ds = -model.hs \ model.gs;
%             [~, model, dat] = lsSigma(model.ds, model, dat, opt);
% 
%             fprintf('sigma: %g\n', model.sigma);
% 
%             % -----------
...
function model = plotAll(model, opt, loop)
    ...
    if isfield(model, 'llr')
        if ~isfield(model, 'savellr')
            model.savellr = [];
        end
        model.savellr = [model.savellr model.llr];
        lb = lb + model.llr;
    end
    ...
    if opt.verbose
        ...
        if isfield(model, 'lbaq')
            ...
            title('Affine precision part')
        end
        if isfield(model, 'llr')
            subplot(px, py, 12)
            plot(model.savellr, 'g-')
            title('Residual part')
        end
...
function [dat, model] = initAll(dat, model, opt)
    ...
    [dat, model] = pgraBatchProcess('InitAffine', 'zero', dat, model, opt);
    model.Aq = saveOnDisk(model.Aq, eye(size(opt.affine_basis, 3)));
    model.regq = model.Aq;
    
    % --- Zero init of R (Residual)
    model.sigma  = opt.sigma0;
    [dat, model] = pgraBatchProcess('InitResidual', 'zero', dat, model, opt);
    ...
    [dat, model] = pgraBatchProcess('InitLatent', 'zero', dat, model, opt);
    
    % --- Init of subject specific arrays
    dat = pgraBatchProcess('Update', dat, model, opt, ...
    ...
    % ---------------------------
    dat = pgraBatchProcess('Update', dat, model, opt, {'llm'});
    model.llm = 0;
    for n=1:opt.N
        model.llm = model.llm + dat(n).llm;
    end
    ...
    % --- Random init of E[z]
    [dat, model] = pgraBatchProcess('InitLatent', 'rand', dat, model, opt);

    % --- Orthogonalise sum{E[z]E[z]'}
    [U,S] = svd(model.zz);
    Rz    = 0.1*sqrt(opt.N/opt.K)*U/diag(sqrt(diag(S)+eps));
    dat   = pgraBatchProcess('RotateLatent', dat, opt, Rz');
