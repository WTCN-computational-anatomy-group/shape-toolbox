function [Q, iQ, q] = pgva_scale_pg(WW, ZZ, SZ, l, A0, n0, N, q0)
...
% Gauss-Newton optimisation of the scaling factor between LW and Z
    ...
    E    = 0.5*(trace(Q*ZZ*Q*A) + trace(WW/(Q*Q)) + l*trace(SZ*(Q\(WW/Q))));
%     fprintf('\n%d %g %g %g %g\n', 0, 0.5*trace(Q*ZZ*Q*A), 0.5*trace(WW*inv(Q*Q)), l*trace(SZ*(Q\(WW/Q))), E)
    ...
    for iter=1:100
        ...
        for subit=1:10
            ...
            g1 = Q*R*diag(Q);
            g2 =-2*(Q^2\diag(WW));
            g3 = -Q^2\(l*diag(WW*diag(diag(SZ))))*2;
            g  = g1+g2+g3;

            H1 = Q*R*Q + diag(g1);
            H2 = 4*(Q^2\WW);
            H3 = 4*l*(WW*diag(diag(SZ)))*Q^(-2);
            H  = H1+H2+H3;
