function [opt, dat, model] = shape_data(opt, dat, model)
% FORMAT [opt, dat, model] = shape_data(opt, dat, model)
%
% Initialise all data handling structures
    ...
    if ~isfield(opt.tpl, 'lat')
        ...
        dims = toList(dat, '.f.f', 'function', @(X) [size(X) 1], 'missing', []);
        Ms   = toList(dat, '.f.M', 'missing', []);
        [opt.tpl.lat, opt.tpl.M, opt.tpl.vs] = autoTemplateLattice(dims, Ms, opt.tpl.vs);
    end
    if ~isfield(opt.tpl, 'vs')
