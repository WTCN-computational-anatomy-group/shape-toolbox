...
% field accounting for the "geodesic noise".
%
...
%             (q, A, z, r, v, llm, llq, f, mu, gmu, iphi, iphi, pf, c, (a))
% It can also be used as input to specify initial starting points.
    ...
    if ~isfield(opt.fnames, 'phi')
        opt.fnames.phi = fullfile(opt.directory, 'diffeo_warp.nii');
    end
    if ~isfield(opt.fnames, 'jac')
        opt.fnames.jac = fullfile(opt.directory, 'diffeo_jacobian.nii');
    end
    ...
    if ~isfield(opt.dat, 'phi')
        lat = [size(opt.dat.mu) 1];
        lat = lat(1:3);
        opt.dat.phi = file_array(opt.fnames.phi, [lat 3], 'float32');
        n = nifti;
        n.dat = opt.dat.phi;
        create(n);
    end
    if ~isfield(opt.dat, 'jac')
        lat = [size(opt.dat.mu) 1];
        lat = lat(1:3);
        opt.dat.jac = file_array(opt.fnames.jac, [lat 3 3], 'float32');
        n = nifti;
        n.dat = opt.dat.jac;
        create(n);
    end
    ...
    % Reconstruct complete mapping field
    % ----------------------------------
    opt.dat.ipsi = reconstructIPsi(opt.dat.A, opt.dat.iphi, ...
    ...
    [opt.dat.phi, opt.dat.jac] = exponentiateVelocity(opt.dat.v, 'phi', 'jac', ...
        'itgr', opt.itgr, 'vs', sqrt(sum(opt.dat.Mmu(1:3,1:3).^2)), ...
        'prm', opt.prm, 'debug', opt.debug, 'output', {opt.dat.phi, opt.dat.jac});
    ...
    for gnit = 1:opt.gnit
        ...
        [opt.dat.gq, opt.dat.hq] = ghMatchingAffine(opt.model, ...
            ...
            opt.dat.phi, opt.dat.jac, ...
            'Mmu', opt.dat.Mmu, 'loop', opt.loop, 'par', opt.par, ...
            'debug', opt.debug, 'approx', opt.happrox);
        
        if checkarray(opt.regq)
            [gq, hq] = ghPriorAffine(opt.dat.q, opt.regq, 'debug', opt.debug);
            opt.dat.gq = opt.dat.gq + gq;
        ...
        % Line search
        % -----------
        [okq, q, llm, llq, A, pf, c, ipsi] = lsAffine(...
            opt.model, opt.dat.dq, opt.dat.q, opt.dat.llm, opt.dat.mu, opt.dat.f, ...
            'B', opt.affine_basis, 'regq', opt.regq, 'iphi', opt.dat.iphi, ...
        ...
        [okr, r, llm, llr, iphi, pf, c, ipsi, v] = lsVelocity(...
            ...
            opt.dat.mu, opt.dat.f, 'v0', opt.dat.v, 'A', opt.dat.A, ....
            'Mf', opt.dat.Mf, 'Mmu', opt.dat.Mmu, 'nit', opt.lsit, ...
            'itgr', opt.itgr, 'prm', opt.prm, ...
            'par', opt.par, 'verbose', opt.verbose, 'debug', opt.debug);
        ...
        % ------------------------------------
        % 4. For affine, direct diffeomorphism
        % ------------------------------------
        [opt.dat.phi, opt.dat.jac] = exponentiateVelocity(opt.dat.v, 'phi', 'jac', ...
            'itgr', opt.itgr, 'vs', sqrt(sum(opt.dat.Mmu(1:3,1:3).^2)), ...
            'prm', opt.prm, 'debug', opt.debug, 'output', {opt.dat.phi, opt.dat.jac});
